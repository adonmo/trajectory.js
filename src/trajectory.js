...
var BIG_1 = BigInt(1);
var BIG_5 = BigInt(5);
var BIG_0x20 = BigInt(0x20);
var BIG_0x1f = BigInt(0x1f);
var BIG_63 = BigInt(63);
...
function encode(current, previous, factor) {
    ...
    var coordinate = BigInt(current - previous);
    coordinate <<= BIG_1;
    if (current - previous < 0) {
        coordinate = ~coordinate;
    }
    var output = '';
    while (coordinate >= BIG_0x20) {
        output += String.fromCharCode(Number((BIG_0x20 | (coordinate & BIG_0x1f)) + BIG_63));
        coordinate >>= BIG_5;
    }
    output += String.fromCharCode(Number(coordinate + BIG_63));
    return output;
}

var _trans = function(str, index) {
    var shift = 0, result = BIG_0, byte = null;

    do {
        byte = str.charCodeAt(index++) - 63;
        result |= BigInt(byte & 0x1f) << BigInt(shift);
        shift += 5;
    } while (byte >= 0x20);

    return {
        trans: Number((result & BIG_1) ? ~(result >> BIG_1) : (result >> BIG_1)),
        index: index
    };
}
...
trajectory.encode = function(traj, precision) {
    ...
    var factor = Math.pow(10, Number.isInteger(precision) ? precision : 5),
        output = encode(traj[0][0], 0, factor) + encode(traj[0][1], 0, factor) + encode(traj[0][2], 0, factor);

    for (var i = 1; i < traj.length; i++) {
        var a = traj[i], b = traj[i - 1];
